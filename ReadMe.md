...
For some reason, there has never been a pixel-perfect Amiga-native version of a double-height Topaz (at least not that we could find) for use with resolutions with a square (1:1) pixel ratio, so we made one.
